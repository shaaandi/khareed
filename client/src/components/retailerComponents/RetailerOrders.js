...
class RetailerOrders extends Component{
    ...
    render () {
        ...
        return (
            [
                <Route 
                exact path='/retailer/orders'
                render={() => {
                    let orders = this.state.orders.map(order => {
                        return (
                            <div className='order'>
                                <h3><span>Order Id : </span>{order._id}</h3>
                                <h3><span>Total Amount : </span>{order.tAmount}</h3>
                ...
                render={({match}) => {
                    return (
                        <RetailerOrder id={match.params.id} />
                    )
                }}
                />
            ]
...
export default connect(null, {fetchRetailerOrders})(withRouter(RetailerOrders))
...
class Category extends Component{
        ...
        if(obj){
            this.setState({
                filters : {
                    ...
                    sortField : (obj.sortField !== 'undefined') ? obj.sortField : 'customer_average_review_rating',
    ...
    handleSubmit = async (e) => {
        e.preventDefault()
        let {subCategory, brand, minPrice, maxPrice, sortField, sortOrder} = this.state.filters;
        ...
        let filters = `subCategory=${encodedSubCategory || undefined}&brand=${encodedBrand || undefined}&minPrice=${minPrice}&maxPrice=${maxPrice}&sortField=${sortField}&sortOrder=${sortOrder}&pageNum=${1}`
    ...
    handleSorting = async (e) => {
        ...
        let {subCategory, brand, minPrice, maxPrice, sortOrder, sortField} = this.state.filters;
        let encodedSubCategory = subCategory.split('&').join('%26')
        let encodedBrand = brand.split('&').join('%26')
        let filters = `pageNum=1&subCategory=${encodedSubCategory || undefined}&brand=${encodedBrand || undefined}&minPrice=${minPrice}&maxPrice=${maxPrice}&sortField=${sortField}&sortOrder=${sortOrder}`
    ...
    handlePageTransition =  async (mode='', num) => {
        let category = this.props.category;
        let oldPageNum = (this.props.options.pageNum === undefined) ? 1 : parseInt(this.props.options.pageNum)
        let {subCategory, brand, minPrice, maxPrice, sortOrder, sortField} = this.state.filters;
        let encodedSubCategory = subCategory.split('&').join('%26')
        let encodedBrand = brand.split('&').join('%26')
        let preFilters = `subCategory=${encodedSubCategory || undefined}&brand=${encodedBrand || undefined}&minPrice=${minPrice}&maxPrice=${maxPrice}&sortField=${sortField}&sortOrder=${sortOrder}`;
        let newPageNum ;
        ...
        return (
            <form onSubmit={this.handleSubmit} id='categoryFilters'>
                <div id='closeButtonWrapper'>
                    ...
                    <button id='closeButton' onClick={this.toggleFilters}>X</button>
                </div>
    ...
    render (){
        ...
        let fromProduct = (((limitPerPage*(pageNum-1))+1) < totalResultsCount) ? (limitPerPage*(pageNum-1))+1 : totalResultsCount;
        let toProduct = ((limitPerPage*(pageNum)) < totalResultsCount) ? limitPerPage*(pageNum) : totalResultsCount;
        let totalNumberOfPages = Math.trunc(totalResultsCount / 20)+1;
        // Showing Number count string variables 
        let subCategoryString = (this.props.options.subCategory === 'undefined' || this.props.options.subCategory === undefined) ? '' : `in ${this.props.options.subCategory}`;
        let brandString = (this.props.options.brand === 'undefined' || this.props.options.brand === undefined) ? '' : `from ${this.props.options.brand}`;
        // **********************
        ...
        return(
            <div id='CategorySearch'>
                ...
                <div id='filterAndSortingHead'>
                    { (this.state.filterMode) ?  <div>{this.renderForm()}</div> : <button onClick={this.toggleFilters}>Filters</button>}
                    <form id='sortingForm' onSubmit={this.handleSorting}>
                        <select value={this.state.filters.sortField}  onChange={this.handleChange} name='sortField' defaultValue={this.state.filters.sortField}>
                            <option value={'price'}>Price</option>
                            <option value={'customer_average_review_rating'}>Rating</option>
                        </select>
                        <select value={this.state.filters.sortOrder}  onChange={this.handleChange} name='sortOrder' defaultValue={this.state.filters.sortOrder}>
                ...
                <div className='paging'>
                    ...
                    <form id='jumpPageForm' onSubmit={(e) => {
                        e.preventDefault();
...
export default connect(mapStoreToProps, actions)(withRouter(Category))
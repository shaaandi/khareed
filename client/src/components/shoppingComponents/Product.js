...
import {addProductToCart, addProductToWishlist, fetchSimilarProducts} from '../../actions/customerActions';
        ...
        if(this.props.user){
            if(this.props.user.badge === 'CUSTOMER') {
                return ([
                    ...
                    <form className='shopProductCartForm' onSubmit={(e) => {
                        e.preventDefault();
                        this.props.addProductToCart(this.props.id,this.state.cartProductQuantity)
                    }}> 
                        <div className='setters'>
                            <input className='crements'
                    type='button' value='-'
                     onClick={() => {
                         let num;
                        if(this.state.cartProductQuantity > 1) num = this.state.cartProductQuantity - 1
                        else num = 1
                        this.setState({
        ...
        return([
            <div className='shopProduct'>
                ...
                <div className='shopProductData'>
                    ...
                    <div className='shopProductOptions'>
                        {this.renderOptions()}
                    </div>
                </div>
            </div>,
...
export default connect(mapStoreToProps, {fetchProduct, addProductToCart, addProductToWishlist, fetchSimilarProducts})(Product);